...
class PosRepository(
    ...
    suspend fun completeSale(
        ...
        if (payments.isEmpty()) return Result.failure(Exception("No payments provided"))
        ...
        if (houseTabPayment != null && houseTabPayment.amount_cents > 0) {
            if (customerId == null) {
                return Result.failure(Exception("House Tab requires a named customer"))
            }
    ...
    suspend fun processReturn(
        ...
        val saleItem = saleItemDao.getSaleItemById(saleItemId)
            ?: return Result.failure(Exception("Sale item not found"))

        if (mgReturned <= 0) return Result.failure(Exception("Return weight must be greater than 0"))
    ...
    suspend fun voidSale(saleId: Long, reason: String): Result<Unit> {
        ...
        if (sale.status == "VOIDED") {
            return Result.failure(Exception("Sale is already voided"))
        }
